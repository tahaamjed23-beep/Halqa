...
const TOUR: Partial<Record<Page, { title: string; body: string; cta?: { label: string; page: Page } }>> = {
  ...
  market: { title: 'The Turn Market', body: 'Need cash sooner, or happy to wait and earn? Members trade turns here. Halal circles (Sukoon/Bazaar) allow free swaps only.' },
  ...
  profile: { title: 'Your Profile', body: 'Your reliability score, the Sukoon Vault (save any amount and earn), and your shareable Credit Passport all live here.' },
};
...
export default function RafaBot({ page, setPage }: { page: Page; setPage: (page: Page) => void }) {
  ...
  const ask = (raw: string) => {
    ...
    if (hit) say(hit.entry.a, hit.entry.go as Page | undefined);
    else say("I'm not sure about that one yet — I know committees, joining & hosting, the profit engines and the 10% fee, safety & defaults, the vault, the turn market and your account. Try one of those, or tap a suggestion below.");
  };
