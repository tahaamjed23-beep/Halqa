...
import { SHOW_BANK_RAIL, SHOW_HOST_ELIGIBILITY } from '../config';
...
const VAULT_TIER_LABEL:Record<string,string>={STANDARD:'Standard',INCOME:'Income',GOLD:'Gold-linked'};

function VaultPanel(){
  const [vault,setVault]=useState<VaultInfo|null>(null);const [busy,setBusy]=useState(false);const [error,setError]=useState('');const [swept,setSwept]=useState<{principalPaisa:string;profitPaisa:string}|null>(null);const [topup,setTopup]=useState(5000);
  const load=useCallback(()=>api<VaultInfo>('/vault').then(setVault).catch(()=>setVault(null)),[]);
  useEffect(()=>{void load()},[load]);
  const setTier=async(tier:string)=>{setBusy(true);setError('');try{await api('/vault/tier',{method:'POST',body:JSON.stringify({tier})});await load()}catch(reason){setError((reason as Error).message)}finally{setBusy(false)}};
  ...
  <div className="vault-tiers">{vault.tiers.map(t=><button key={t} type="button" className={`vault-tier ${vault.tier===t?'on':''}`} disabled={busy} onClick={()=>setTier(t)}><b>{VAULT_TIER_LABEL[t]||t}</b><small>{t==='STANDARD'?'Money-market · lowest volatility':t==='INCOME'?'Islamic income fund · higher yield':'Gold-linked · halal inflation hedge'}</small></button>)}</div>
