...
await prisma.committee.updateMany({where:{host:{username:'sana'},status:{in:['FORMING','ACTIVE']},OR:[{name:{startsWith:'Lifecycle '}},{name:{startsWith:'Soneri Rail '}},{name:{startsWith:'Bazaar '}},{name:{startsWith:'Priority '}},{name:{startsWith:'Sigma '}},{name:{startsWith:'Swap '}},{name:{startsWith:'Cover '}}]},data:{status:'CANCELLED'}}); // clear stale test circles so the 5-hosted cap never trips
...
// Guarantee requires funding; slot fees require custody
...
check('vault reports the active tier and the choices',vaultView2.tier==='INCOME'&&Array.isArray(vaultView2.tiers)&&vaultView2.tiers.length===3);
