...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const guaranteeFundPaisa = committee.custodyMode === 'BANK_CUSTODY' ? (await guaranteeFundBalance(prisma, committee.id)).toString() : undefined;
...
router.post('/', async (req, res, next) => {
  try {
    const input = z.object({
      name: z.string().trim().min(3).max(80), memberCap: z.number().int().min(3).max(30),
      ...
      cadencePreset: z.enum(['VERY_SHORT','SHORT','MID','LONG']), periodDays: z.number().int().min(7).max(365),
    ...
    // Partner-rail gates: custody, guaranteed payouts and slot fees exist only on
    // the bank rail; the guarantee pool needs the slot fee as its funding source.
    ...
    if (input.custodyMode === 'BANK_CUSTODY') {
      ...
      if (input.payoutGuaranteed && input.slotFeeBps < 50) return res.status(400).json({ error: 'Guaranteed payouts need a slot fee of at least 0.5% to fund the guarantee pool' });
      partnerId = partner.id;
    } else if (input.payoutGuaranteed || input.slotFeeBps > 0) {
      return res.status(400).json({ error: 'Guaranteed payouts and slot fees require bank custody mode' });
    }
...
router.post('/:id/start', async (req, res, next) => {
  try {
    ...
    const ordered = committee.orderMode === 'CREDIT_WEIGHTED'
      ? [...members].sort((a,b) => b.user.creditScore - a.user.creditScore || a.joinedAt.getTime() - b.joinedAt.getTime())
      : members;
    const now = new Date();
    await prisma.$transaction(async tx => {
      ...
      for (let i=0; i<ordered.length; i++) {
        ...
        const positionCoverageBps = ordered.length <= 1 ? 0 : Math.round(7000 * (ordered.length - i - 1) / (ordered.length - 1));
        const creditAdjustmentBps = Math.max(-1000, Math.min(1500, Math.round((700 - ordered[i].user.creditScore) * 10)));
        const requiredCoverageBps = Math.max(0, Math.min(9000, positionCoverageBps + creditAdjustmentBps));
...
router.post('/:id/payout', async (req, res, next) => {
  try {
    ...
    // of +5% per consecutive clean round, capped at +20%. Funded from the same
    // simulated sleeve; rewards discipline, never charges anyone.
    const streakBonusBps = BigInt(500 * Math.min(4, committee.cleanStreak));
