...
## Fourteenth block (2026-07-13) — the full-hour hardening pass (Fable 5)

- **Cleanup:** deleted stale `halqa-api/dist/` (regenerable) and the dead
  RafaGuide re-export shim; index.html got a real title, description,
  theme-color; web src confirmed free of console.logs.
- **Pakistan sim hardened:** core extracted to `tests/lib-pakistan-sim.ts`
  (named calibration constants), printable runner kept, and a new
  `pakistan-sim.test.ts` with **8 CI assertions** — determinism, segment shares
  ±1%, burden band, ≥3× default reduction, >60% recovery, dropout/on-time
  improvements, risk ordering, and second-seed robustness. Headline numbers
  unchanged (1.41%/0% vs 0.27%/85.5%/64%) — the refactor is faithful.
- **Crypto made functional:** vault tier CRYPTO (`crypto-basket`, personal
  vault only). API refuses a silent switch (HTTP 428 without
  `acknowledgeExtremeRisk:true`); UI shows a red tier button ("extreme · not
  Shariah") + a double-confirm modal; committees remain permanently barred.
  Terminal band chips now include Extreme.
- **Change password shipped** (`POST /api/auth/change-password`): verifies the
  current password, revokes all refresh tokens; Profile → Security panel. Rafa's
  password answer updated to the new truth.
- **Phone version:** manifest.webmanifest + PWA meta (installable), Rafa
  becomes a bottom sheet ≤600px, 4-tier vault grid auto-fits, touch targets
  ≥44px, sticky scheme toolbar; verified at 375×812 — no horizontal overflow,
  FAB clears the mobile nav, all key pages walked.
- **Test-infra fixes found by the loop:** stale 3-tier assertion updated; auth
  rate limiter now 30 (prod) / 200 (dev) — three consecutive suite runs exceed
  30 auth calls; suite is now **self-healing** (resets taha's password hash at
  start after a mid-rotation crash locked a run out).
- Suite: **25 unit + 251 integration ×3 consecutive**, typechecks + production
  build clean, desktop + mobile UI walked live (crypto confirm → 30% shown →
  restored; Shariah chips verified on all four tiers).

## Fifteenth block (2026-07-13) — the chairman-approved six-feature batch

- **Deposit coverage raised 70% → 90% (cap 90% → 95%):** the graduated
  security-deposit curve in `committees.ts` now sizes early positions at 90%
  of remaining dues; mirrored in all five sim/test files. Pakistan sim (100k):
  recovery on default **85.5% → 90.4%**, defaults fully collateralised
  **64% → 70%**, post-receipt default rate steady at 0.27%. Side effect on
  earnings (larger Mudarabah deposit base): **Sigma pooled max Rs 16,982**
  (was 15,639), Sigma monthly **Rs 15,285** — the Rs 15,000 promise now clears
  one rung earlier, without pooled mode. Bazaar 7,649, Sukoon 5,736, Priority
  7,637, group total Rs 68,837. Backtest stress floor (rates −30%, due-date
  payers) **Rs 13,821** (was ~12,900); rate-independent fee floor ~Rs 9,334.
- **First-circle cap (graduated exposure):** a member with zero completed
  cycles cannot join a circle above Rs 25,000/installment (HTTP 403 with a
  plain-language explanation). Proven members unaffected.
- **Referral loyalty pool:** when a referred member completes their first full
  cycle, the referrer is credited **Rs 250 from Halqa's own platform-fee
  share** (`platform:fees` → referrer external, idempotencyKey
  `referral:{userId}`, strictly once ever) + notification. Registration
  accepts an optional `referredBy` username (silently ignored if invalid).
  Member money is never touched — halal by construction.
- **Bank receivables pack:** `GET /committees/:id/receivables-pack`
  (host-only) exports future-round inflow schedule, per-member reliability
  (score, KYC, on-time %, collateral held) and a record-only disclaimer;
  "Bank pack" button on the payments tab downloads it as JSON. Built for the
  Ashai-style "financing backed by receivables" conversation.
- **Urdu foundation:** `src/lib/i18n.ts` (typed dictionary, `t()`/`useLang()`,
  RTL flip, persisted). Toggle in the sidebar; nav, home hero, trust banner
  translated first. Bug found live and fixed: per-component language state
  meant sibling pages didn't flip — replaced with a module-level shared
  subscription so every mounted surface re-renders on switch.
- **Rafa LLM scaffold:** on a knowledge-base miss, Rafa POSTs the question to
  `window.RAFA_LLM_URL` if configured (6s timeout, falls back to the honest
  KB miss message). Inert until an endpoint is provided — no key shipped.
- Docs renumbered to the new engine output (reports 01–09, meeting prep,
  Gamma prompt, dossier HTML + regenerated PDF); the dossier's "recommended
  coverage raise" paragraphs now state it shipped.
- Suite: **25 unit + 262 integration ×2 consecutive** (new: referral ledger
  once-ever, first-circle cap 403, receivables pack fields + host-only 403);
  typechecks + production build clean; Urdu toggle and Bank pack export
  verified live in the browser. Test-infra: suite re-login within the same
  second collided on the refresh-token hash (register's token reused instead);
  Newbie circle hosted by ayesha to respect sana's five-circle cap.
- All nine investor reports rebuilt (v2, deeper) in docs/reports/.
