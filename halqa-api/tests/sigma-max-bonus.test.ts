...
function simulate(stage: Stage) {
  ...
  if (stage.engine) {
    ...
    for (let pos = 1; pos <= MEMBERS; pos++) {
      ...
      const coverageBps = Math.max(0, Math.min(7500, Math.round(7000 * (MEMBERS - pos) / (MEMBERS - 1))));
...
describe('the 12-women reference group reaches a Rs 15,000 max bonus with the fee fixed at 10%', () => {
  ...
  it('climbs the ladder until the goal is met, without ever raising the fee', () => {
    ...
    // Deposit coverage is now host-configurable (30–90%); this asserts the
    // DEFAULT (70%) still clears the Rs 15,000 promise — which it does at the
    // full Sigma-pooled configuration (pooled dividends run through the tilt).
