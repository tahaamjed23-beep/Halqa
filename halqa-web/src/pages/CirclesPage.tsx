...
export default function CirclesPage({user,openCommittee,create}:{user:User;openCommittee:(id:string)=>void;create:()=>void}){
  ...
  const doJoinPublic=async(id:string)=>{try{setError('');setBusy(true);await api(`/committees/${id}/join-public`,{method:'POST',body:'{}'});await load();openCommittee(id)}catch(reason){setError((reason as Error).message)}finally{setBusy(false)}};
  const join=()=>{if(preview)setPgTarget({committeeId:preview.id,run:doJoin})};
  const joinPublic=(id:string)=>setPgTarget({committeeId:id,run:()=>doJoinPublic(id)});
