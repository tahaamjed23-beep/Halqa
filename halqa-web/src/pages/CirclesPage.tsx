...
export default function CirclesPage({user,openCommittee,create}:{user:User;openCommittee:(id:string)=>void;create:()=>void}){
  const [rows,setRows]=useState<Committee[]>(()=>cached('circles.rows',[]));const [code,setCode]=useState('');const [error,setError]=useState('');const [preview,setPreview]=useState<Preview|null>(null);const [busy,setBusy]=useState(false);
  const load=()=>api<Committee[]>('/committees').then(data=>{setRows(data);keep('circles.rows',data)});useEffect(()=>{void load()},[]);
