...
function ChangePasswordPanel(){
  ...
  return <section className="panel"><div className="panel-head"><div><span className="eyebrow">Security</span><h2>Change password</h2><p>Updating your password signs out every other session.</p></div><Fingerprint/></div>
  <div className="form-grid"><Field label="Current password"><input className="field" type="password" value={current} onChange={e=>setCurrent(e.target.value)}/></Field><Field label="New password (min 8 characters)"><input className="field" type="password" value={next} onChange={e=>setNext(e.target.value)}/></Field></div>
  {done&&<div className="commitment-ok" style={{marginTop:10}}><ShieldCheck/><div><b>Password updated</b><p>Other sessions were signed out.</p></div></div>}
  {error&&<div className="error-box">{error}</div>}
  <div className="form-actions"><button className="primary" disabled={busy||current.length<8||next.length<8} onClick={submit}>{busy?'Updating…':'Update password'}</button></div></section>;
}
