...
import { Empty, HalqaOrb, Metric, Mini, ScoreRing, TurnPricingChip, formatDuration, pricingOf } from '../components/ui';
...
export default function HomePage({user,openCommittee,create}:{user:User;openCommittee:(id:string)=>void;create:()=>void}){
  ...
  useEffect(()=>{void Promise.all([api<Committee[]>('/committees?scope=mine'),api<Summary>('/profile/summary')]).then(([rows,data])=>{const mine=rows.filter(row=>row.members?.some(member=>member.userId===user.id)||row.hostId===user.id);setCommittees(mine);setSummary(data);keep('home.committees',mine);keep('home.summary',data)})},[user.id]);
